        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '性别' : ''">
                        <el-select v-model="searchForm.xingbieTypes" placeholder="请选择性别">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in xingbieTypesSelectSearch"
                               v-bind:key="index"
                               :label="item.indexName"
                               :value="item.codeIndex">
                            <!--lable是要展示的名称-->
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '动物状态' : ''">
                        <el-select v-model="searchForm.dongwuZhuangtaiTypes" placeholder="请选择动物状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in dongwuZhuangtaiTypesSelectSearch"
                               v-bind:key="index"
                               :label="item.indexName"
                               :value="item.codeIndex">
                            <!--lable是要展示的名称-->
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('liulangdongwu','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/liulangdongwujiuzhu/upload/liulangdongwuMuBan.xls"
                        >批量导入流浪动物数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('liulangdongwu','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="liulangdongwuUploadSuccess"
                                :on-error="liulangdongwuUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('liulangdongwu','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入流浪动物数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('liulangdongwu','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "liulangdongwu.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="liulangdongwuAge"
                                      header-align="center"
                                      label="流浪动物年龄">
                        <template slot-scope="scope">
                            {{scope.row.liulangdongwuAge}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="liulangdongwuJiezhong"
                                   header-align="center"
                                   label="疫苗接种">
                        <template slot-scope="scope">
                            {{scope.row.liulangdongwuJiezhong}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="xingbieTypes"
                                      ...
                                      label="性别">
                        <template slot-scope="scope">
                            {{scope.row.xingbieValue}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="liulangdongwuZuyangjin"
                                   header-align="center"
                                   label="助养金">
                        <template slot-scope="scope">
                            {{scope.row.liulangdongwuZuyangjin}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="dongwuZhuangtaiTypes"
                                      header-align="center"
                                      label="动物状态">
                        <template slot-scope="scope">
                            {{scope.row.dongwuZhuangtaiValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('liulangdongwu','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('liulangdongwu','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('liulangdongwu','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                id : null,
                liulangdongwuName : null,
                liulangdongwuPhoto : null,
                liulangdongwuTypes : null,
                liulangdongwuErjiTypes : null,
                liulangdongwuAge : null,
                liulangdongwuJiezhong : null,
                xingbieTypes : null,
                liulangdongwuZuyangjin : null,
                liulangdongwuContent : null,
                dongwuZhuangtaiTypes : null,
                liulangdongwuDelete : null,
                createTime : null,
            },
            dataList: [],
            pageIndex: 1,
            ...
            json_fields: {
                ...
                //本表字段
                     ...
                     '动物类型': "liulangdongwuValue",
                     '详细类型': "liulangdongwuErjiValue",
                     '流浪动物年龄': "liulangdongwuAge",
                     '疫苗接种': "liulangdongwuJiezhong",
                     '性别': "xingbieValue",
                     '助养金': "liulangdongwuZuyangjin",
                     '动物状态': "dongwuZhuangtaiValue",
            },
        ...
        methods: {
            chartDialog() {
                ...
                this.$nextTick(()=>{
                    ...
                    let params = {
                        ...
                        groupColumn: "liulangdongwu_types",
                    }
                    this.$http({
            ...
            getDataList() {
                ...
                params['liulangdongwuDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "liulangdongwu/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            liulangdongwuUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "liulangdongwu/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
