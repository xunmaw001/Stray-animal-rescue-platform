        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '花费标题' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.liulangdongwuHuafeiName" placeholder="花费标题" clearable></el-input>
                     </el-form-item>
                         
                     <el-form-item :label="contents.inputTitle == 1 ? '消费类型' : ''">
                        <el-select v-model="searchForm.liulangdongwuHuafeiTypes" placeholder="请选择消费类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in liulangdongwuHuafeiTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('liulangdongwuHuafei','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('liulangdongwuHuafei','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('liulangdongwuHuafei','报表')"
                                type="success"
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('liulangdongwuHuafei','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/liulangdongwujiuzhu/upload/liulangdongwuHuafeiMuBan.xls"
                        >批量导入流浪动物花费数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('liulangdongwuHuafei','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="liulangdongwuHuafeiUploadSuccess"
                                :on-error="liulangdongwuHuafeiUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('liulangdongwuHuafei','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入流浪动物花费数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('liulangdongwuHuafei','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "liulangdongwuHuafei.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('liulangdongwuHuafei','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="liulangdongwuHuafeiUuidNumber"
                                   header-align="center"
                                   label="流浪动物花费编号">
                        <template slot-scope="scope">
                            {{scope.row.liulangdongwuHuafeiUuidNumber}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="liulangdongwuHuafeiName"
                                   header-align="center"
                                   label="花费标题">
                        <template slot-scope="scope">
                            {{scope.row.liulangdongwuHuafeiName}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="liulangdongwuHuafeiFile"
                                     header-align="center"
                                     width="200"
                                     label="附件">
                        <template slot-scope="scope">
                            <div v-if="scope.row.liulangdongwuHuafeiFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.liulangdongwuHuafeiFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="liulangdongwuHuafeiMoney"
                                   header-align="center"
                                   label="花费金额">
                        <template slot-scope="scope">
                            {{scope.row.liulangdongwuHuafeiMoney}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="liulangdongwuHuafeiTypes"
                                      header-align="center"
                                      label="消费类型">
                        <template slot-scope="scope">
                            {{scope.row.liulangdongwuHuafeiValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('liulangdongwuHuafei','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('liulangdongwuHuafei','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('liulangdongwuHuafei','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              liulangdongwuHuafeiTypesSelectSearch : [],
            form:{
                id : null,
                liulangdongwuId : null,
                liulangdongwuHuafeiUuidNumber : null,
                liulangdongwuHuafeiName : null,
                liulangdongwuHuafeiFile : null,
                liulangdongwuHuafeiMoney : null,
                liulangdongwuHuafeiTypes : null,
                huafeiTime : null,
                liulangdongwuHuafeiContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                ...
                //本表字段
                     '流浪动物花费编号': "liulangdongwuHuafeiUuidNumber",
                     '花费标题': "liulangdongwuHuafeiName",
                     '附件': "liulangdongwuHuafeiFile",
                     '花费金额': "liulangdongwuHuafeiMoney",
                     '消费类型': "liulangdongwuHuafeiValue",
                     '花费时间': "huafeiTime",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'liulangdongwu_huafei',//当前表表名,
                        sumColum : 'liulangdongwu_huafei_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'liulangdongwu_huafei_name',//分组字符串字段
                        // types : 'liulangdongwu_huafei_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "liulangdongwu_huafei_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.liulangdongwuHuafeiName!= '' && this.searchForm.liulangdongwuHuafeiName!= undefined) {
                    params['liulangdongwuHuafeiName'] = '%' + this.searchForm.liulangdongwuHuafeiName + '%'
                }
                         
                if (this.searchForm.liulangdongwuHuafeiTypes!= '' && this.searchForm.liulangdongwuHuafeiTypes!= undefined) {
                    params['liulangdongwuHuafeiTypes'] = this.searchForm.liulangdongwuHuafeiTypes
                }
                                
                params['liulangdongwuHuafeiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "liulangdongwuHuafei/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=liulangdongwu_huafei_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.liulangdongwuHuafeiTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            deleteHandler(id) {
                ...
                this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
                    ...
                    this.$http({
                        url: "liulangdongwuHuafei/delete",
                        method: "post",
                        data: ids
                    }).then(({data}) => {
                        if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            liulangdongwuHuafeiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "liulangdongwuHuafei/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
                            message: "导入流浪动物花费数据成功",
                            type: "success",
                            duration: 1500,
                            onClose: () => {
                                _this.search();
            ...
            // 导入功能上传文件失败后调用导入方法
            liulangdongwuHuafeiUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "liulangdongwuHuafei/update",
                        method: "post",
                        data: {
                            id:id,
