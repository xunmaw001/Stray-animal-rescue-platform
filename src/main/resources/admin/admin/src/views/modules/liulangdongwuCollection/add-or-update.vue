    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="类型" prop="liulangdongwuCollectionTypes">
                        <el-select v-model="ruleForm.liulangdongwuCollectionTypes" :disabled="ro.liulangdongwuCollectionTypes" placeholder="请选择类型">
                            <el-option
                                v-for="(item,index) in liulangdongwuCollectionTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="类型" prop="liulangdongwuCollectionValue">
                        <el-input v-model="ruleForm.liulangdongwuCollectionValue"
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    ...
                    yonghuId: false,
                    liulangdongwuCollectionTypes: false,
                    insertTime: false,
                },
                ruleForm: {
                    liulangdongwuId: '',
                    yonghuId: '',
                    liulangdongwuCollectionTypes: '',
                    insertTime: '',
                },
                liulangdongwuCollectionTypesOptions : [],
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=liulangdongwu_collection_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.liulangdongwuCollectionTypesOptions = data.data.list;
                    }
                });
        ...
        methods: {
            ...
            info(id) {
                let _this =this;
                _this.$http({
                    url: `liulangdongwuCollection/info/${id}`,
                    method: 'get'
                }).then(({ data }) => {
                    if (data && data.code === 0) {
            ...
            onSubmit() {
                this.$refs["ruleForm"].validate(valid => {
                    if (valid) {
                        this.$http({
                            url:`liulangdongwuCollection/${!this.ruleForm.id ? "save" : "update"}`,
                            method: "post",
                            data: this.ruleForm
                        }).then(({ data }) => {
                            if (data && data.code === 0) {
                                this.$message({
                                    ...
                                    onClose: () => {
                                        ...
                                        this.parent.liulangdongwuCollectionCrossAddOrUpdateFlag = false;
                                        this.parent.search();
